...
import java.io._
import com.wajam.nrv.consistency.persistence2.LogRecord._
...
//    appendMeter.mark()
  ...
  private[persistence2] def getNameFromIndex(index: Index): String = {
    "%s-%d:%s.log".format(filePrefix, index.id, index.consistentTimestamp.getOrElse("").toString)
  }

  private[persistence2] def getIndexFromName(name: String): Index = {
