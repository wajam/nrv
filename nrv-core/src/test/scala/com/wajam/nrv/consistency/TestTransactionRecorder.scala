...
class TestTransactionRecorder extends TestTransactionBase with BeforeAndAfter with MockitoSugar {
  ...
  test("consistent timestamp should not have a transaction with a greater timestamp appended before") {
    ...
    appendMessage(20, request4)
    appendMessage(30, createResponseMessage(request4))
    appendMessage(40, createResponseMessage(request1))
    appendMessage(50, request2)
    appendMessage(60, request3)
    appendMessage(1050, request6)
    ...
    verify(txLogProxy.mockAppender).append(LogRecord(20, None, request4))
    verify(txLogProxy.mockAppender).append(LogRecord(30, None, createResponseMessage(request4)))
    verify(txLogProxy.mockAppender).append(LogRecord(40, None, createResponseMessage(request1)))
    verify(txLogProxy.mockAppender).append(LogRecord(50, None, request2))
    verify(txLogProxy.mockAppender).append(LogRecord(60, None, request3))
    verify(txLogProxy.mockAppender).append(LogRecord(1050, Some(1), request6))
