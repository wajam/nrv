...
class TestCluster extends FunSuite with BeforeAndAfter {
  val cluster = new Cluster(new LocalNode("127.0.0.1", Map("nrv" -> 12345, "dummy" -> 12346)), new StaticClusterManager)
  ...
  test("url resolving") {
    ...
    val resolvedAct2 = cluster.getAction(new ActionURL(srv1.name, "/testdef"), method)
    assert(resolvedAct2 === act2, "Got " + resolvedAct2)

    val resolvedAct3 = cluster.getAction(new ActionURL(srv2.name, "/testabc"), method)
    assert(resolvedAct3 === act3, "Got " + resolvedAct3)
  }

  test("action registered with ANY should resolve with a method") {
    val resolvedAct = cluster.getAction(new ActionURL(srv1.name, "/testabc"), ActionMethod(method))
    assert(resolvedAct === act1, "Got " + resolvedAct)
  }
